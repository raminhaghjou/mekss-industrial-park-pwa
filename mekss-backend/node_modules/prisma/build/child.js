...
async function check(state) {
  ...
  // format the URL
  const urlObj = Url__default["default"].parse(state.endpoint, true);
  const basepath = (urlObj.pathname || '').replace(/\/$/, '');
  urlObj.pathname = `${basepath}/v1/check/${state.product}`;
  urlObj.query = {
  ...
  if (process.env.CHECKPOINT_DEBUG_STDOUT !== undefined) {
    ...
    process.stdout.write(JSON.stringify(urlObj, null, '  '));
    return
  }

  // send the request
  const response = await fetch(Url__default["default"].format(urlObj), {
    method: 'get',
